...
def health_increase_labels(health_values: Iterable[int], horizon: int = POSITIVE_REWARD_HORIZON) -> list[int]:
    health_values = list(health_values)
    labels = []
    for step in range(len(health_values)):
        current = health_values[step]
...
def collect_dataset(args, device: th.device) -> Dict[str, th.Tensor]:
    ...
    for episode_idx in range(args.num_episodes):
        ...
        episode_health_increase_10 = health_increase_labels(
            episode_health_labels,
            horizon=POSITIVE_REWARD_HORIZON,
        )
        ...
        health_increase_10_labels.extend(episode_health_increase_10)
    ...
    dataset = {
        ...
        "health_increase_10": th.tensor(health_increase_10_labels, dtype=th.long),
    ...
    print(
        ...
        f"health_increase_10 positives: {int(dataset['health_increase_10'].sum().item())}"
    )
    return dataset
