...
def save_figure(path: str, rows: List[Dict], observations: List[np.ndarray], episode_id: int, step_id: int):
    n = len(rows)
    ...
    ax_bar.set_title(f"Inventory progression counterfactuals, episode {episode_id}, step {step_id}")
...
def main():
    ...
    save_figure(fig_path, rows, observations, args.episode_id, args.step_id)
