...
class PPOImpalaDinoHybridModel(BaseModel):
    def __init__(
        self,
        ...
        dino_readout_type: str = "patch_mean",
        freeze_dino_backbone: bool = True,
        dino_unfreeze_last_n_blocks: int = 0,
        ...
        self.cnn_enc = ImpalaCNN(
            obs_shape,
            dense_init_norm_kwargs=dense_init_norm_kwargs,
            **impala_kwargs,
        )
        self.dino_enc = DinoV3Encoder(
            model_name=dino_model_name,
            image_size=dino_image_size,
            freeze_backbone=freeze_dino_backbone,
            unfreeze_last_n_blocks=dino_unfreeze_last_n_blocks,
        ...
        cnn_dim = impala_kwargs["outsize"]
        hybrid_dim = cnn_dim + self.dino_enc.output_dim
    ...
    def encode(self, obs: th.Tensor) -> th.Tensor:
        z_cnn = self.cnn_enc(obs)
        z_dino = self.dino_enc(obs)
        z = th.cat([z_cnn, z_dino], dim=-1)
        z = self.linear(z)
        return z
    ...
    def get_param_groups(self):
        backbone_params = list(self.dino_enc.trainable_backbone_parameters())
