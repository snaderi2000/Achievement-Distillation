...
class PPOMFRLModel(BaseModel):
    ...
    def __init__(
        ...
        self.zsize = math.prod(self.enc.final_spatial_shape)
        if hidsize != self.zsize:
            raise ValueError(
                f"PPOMFRLModel expects hidsize to match raw flattened IMPALA output ({self.zsize}), got {hidsize}."
            )
        self.hidsize = hidsize
        self.rnn_hidsize = rnn_hidsize
        self.rnn_input_norm = nn.LayerNorm(self.zsize)
        self.rnn_input_proj = nn.Linear(self.zsize, rnn_hidsize)
