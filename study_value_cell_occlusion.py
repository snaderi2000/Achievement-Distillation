...
def save_heatmap(candidate: CandidateState, deltas: np.ndarray, inventory_deltas: np.ndarray, output_path: str, title: str):
    ...
    axes[0].axis("off")
    axes[0].set_xlim(-0.5, candidate.obs.shape[1] - 0.5)
    axes[0].set_ylim(candidate.obs.shape[0] - 0.5, -0.5)
    draw_grid(axes[0], candidate.obs, grid_shape)
    draw_inventory_grid(axes[0], candidate.obs)

    axes[1].imshow(candidate.obs)
    axes[1].imshow(masked_heatmap, cmap="coolwarm", alpha=0.62, vmin=-vmax, vmax=vmax)
    axes[1].set_title(title)
    axes[1].axis("off")
    axes[1].set_xlim(-0.5, candidate.obs.shape[1] - 0.5)
    axes[1].set_ylim(candidate.obs.shape[0] - 0.5, -0.5)
    draw_grid(axes[1], candidate.obs, grid_shape)
    draw_inventory_grid(axes[1], candidate.obs)

    im = axes[2].imshow(masked_heatmap, cmap="coolwarm", vmin=-vmax, vmax=vmax)
    axes[2].set_title("Heatmap only")
    axes[2].axis("off")
    axes[2].set_xlim(-0.5, candidate.obs.shape[1] - 0.5)
    axes[2].set_ylim(candidate.obs.shape[0] - 0.5, -0.5)
    draw_grid(axes[2], candidate.obs, grid_shape)
    draw_inventory_grid(axes[2], candidate.obs)
    fig.colorbar(im, ax=axes[2], shrink=0.78, label="V(occluded) - V(base)")
